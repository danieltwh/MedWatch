...
class FallDetection:
    def __init__(self):
        ...
        self.model = YOLO('yolov8n-pose.pt')  # Initiation of YOLOv8 model
        self.weights = {
            'NOSE': 0.07,
            'LEFT_EYE': 0.07,
            'RIGHT_EYE': 0.07,
        ...
        fall_path = 'fall_logo.png'
        print(fall_path)
        self.fall_logo = cv2.imread(fall_path)
        self.size = 50
    ...
    def get_body_coordinates_from_frame(self, frame):
        results = self.model(frame, conf=0.6, classes=0)
        return (results[0].keypoints.xyn[0].numpy(), results[0].boxes.xyxy[0].numpy())  # 1 Person for now
    ...
    def detect_fall(self, frame):
        ...
        if len(self.CoG_history) >= 2:
            ...
            other_body_parts = ['LEFT_SHOULDER', 'RIGHT_SHOULDER',
